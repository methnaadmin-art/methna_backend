...
async function run() {
  try {
    ...
    for (const status of ['suspended', 'pending_verification', 'rejected', 'banned']) {
      ...
      const adminStatus = adminData?.user?.status ?? adminData?.status;
      addCheck('moderation_sync', `Admin view reflects ${status.toUpperCase()} state`, adminView.ok && adminStatus === status, {
    ...
    const adminStatusAfterRestore = adminAfterRestoreData?.user?.status ?? adminAfterRestoreData?.status;
    addCheck('moderation_sync', 'Admin view shows ACTIVE after restore', adminAfterRestore.ok && adminStatusAfterRestore === 'active', {
      status: adminAfterRestore.status,
      adminStatus: adminStatusAfterRestore,
    });
    ...
    const adminUser = adminDetailData?.user ?? adminDetailData;
    addCheck('profile_settings_sync', 'Admin sees updated user names', adminDetail.ok && adminUser?.firstName === 'RuntimeUpdated' && adminUser?.lastName === 'Profile', {
